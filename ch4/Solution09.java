...
class TreeNode {
    ...
    TreeNode() { val = 0; isVisit = false; left = right = null;}
    ...
    public void add(int x) {
        if (x < val) {
            if (left != null) left.add(x);
            else left = new TreeNode(x, null, null);
        } else {
            if (right != null) right.add(x);
            else right = new TreeNode(x, null, null);
        }
    }
...
class Solution {
    ...
    public void find(List<Integer> res) {
        ...
        for (int i=0; i<treeNodes.size(); i++) {
            ...
            if (node.isVisit) {
                node.isVisit = false;
                if (node.left != null) node.left.isVisit = true;
                if (node.right != null) node.right.isVisit = true;
                res.add(node.val);
                find(res);
                res.remove(res.size()-1);
                if (node.right != null) node.right.isVisit = false;
                if (node.left != null) node.left.isVisit = false;
                node.isVisit = true;
            }
        }
    }
