...
public class codingbootcamp {
    ...
    public void dfs(TreeNode now, long min, long max) {
        if (flag == true) {
            ...
            if (now.left != null) dfs(now.left, min, now.val);
            if (now.right != null) dfs(now.right, now.val, max);
        }
    }
	...
	//Problem 4.2
	public TreeNode createBSTminimumHight(TreeNode root, int [] array, boolean isLeft, int left, int right) {
	...
	public static void main(String[] args) {
		...
		TreeNode BSTroot = s.createBSTminimumHight(null, a, true, 0, a.length - 1);
